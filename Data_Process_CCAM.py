...
Subset_Filtered_Order_Data_Commodity_Code = Filtered_Order_Data[['phone_no', 'commodity_code']]
Subset_Filtered_Order_Data_Commodity_Code =  Subset_Filtered_Order_Data_Commodity_Code.reset_index(drop=True)
# Important

D = np.tile(1, (len(Subset_Filtered_Order_Data_Commodity_Code), 1))
D = pd.DataFrame(D)
Result_temp = pd.concat([Subset_Filtered_Order_Data_Commodity_Code, D], axis = 1)
User_Item_Table = pd.pivot_table(Result_temp, index = 'phone_no', columns = 'commodity_code', aggfunc = np.sum, fill_value = 0)
# The final table for user-item
# Why need D exist?

User_Item_Table.to_csv('User_Item_subset.csv', sep=',', header=True, index=True)
...
D = np.tile(1, (len(Subset_Filtered_Order_Data_User_Two_Category), 1))
D = pd.DataFrame(D)
Subset_Filtered_Order_Data_User_Two_Category = Subset_Filtered_Order_Data_User_Two_Category.reset_index(drop=True)
Subset_Filtered_Order_Data_User_Two_Category = pd.concat([Subset_Filtered_Order_Data_User_Two_Category, D], axis = 1)
...
D = np.tile(1, (len(Subset_Finish_Time_Data_Time_Consumption), 1))
D = pd.DataFrame(D)
Subset_Finish_Time_Data_Time_Consumption =  Subset_Finish_Time_Data_Time_Consumption.reset_index(drop=True)
Subset_Finish_Time_Data_Time_Consumption = pd.concat([Subset_Finish_Time_Data_Time_Consumption, D], axis = 1)
...
User_Profile.to_csv('User_Profile_subset.csv', sep=',', header=True, index=True)

Item_Profile_Data = Order_Data[['commodity_code', 'two_category_name']]
...
Item_Profile = pd.pivot_table(Item_Profile_Data, index = 'commodity_code', columns = 'two_category_name', aggfunc = np.sum, fill_value = 0)

Item_Profile.to_csv('Item_Profile_subset.csv', sep=',', header=True, index=True)
